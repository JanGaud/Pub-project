<script>
	...
	// Accessing the logo from Prismic data
	const logo = $page.data.nav.data.logo;

	// Accessing the links array from Prismic data
	const prismicLinks = $page.data.nav.data.link || []; // Fallback to an empty array if undefined
...
<div
	...
	} transition-transform duration-300 ease-in-out md:hidden z-40 bg-gold-gradient backdrop-blur-md`}
>
	<ul
